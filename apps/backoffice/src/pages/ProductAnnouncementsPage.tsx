...
export default function ProductAnnouncementsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (value: Draft) => {
      ...
      if (value.expires_at && value.publish_at && new Date(value.expires_at) <= new Date(value.publish_at)) {
      ...
      const payload = {
        ...
        status: value.status,
        publish_at: value.status === "published" ? (toIso(value.publish_at) ?? new Date().toISOString()) : toIso(value.publish_at),
        ...
        {showEditor && (
          <Card>
            ...
            <CardContent className="grid gap-5 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2"><Label>Title</Label><Input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="Automated reminders are here" /></div>
              <div className="space-y-2 md:col-span-2"><Label>Summary</Label><Input value={draft.summary} onChange={(event) => setDraft({ ...draft, summary: event.target.value })} placeholder="Send timely appointment reminders automatically." /></div>
              ...
              <div className="space-y-2"><Label>Publish date</Label><Input type="datetime-local" value={draft.publish_at} onChange={(event) => setDraft({ ...draft, publish_at: event.target.value })} /></div>
              <div className="space-y-2"><Label>Expiry date (optional)</Label><Input type="datetime-local" value={draft.expires_at} onChange={(event) => setDraft({ ...draft, expires_at: event.target.value })} /></div>
              <div className="space-y-3"><Label>Status</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value as Status })}><option value="draft">Draft</option><option value="scheduled">Scheduled</option><option value="published">Published now</option><option value="archived">Archived</option></select></div>
              <div className="space-y-3"><Label>Audience</Label><div className="grid gap-2 text-sm">{([['salon_admin', 'Salon Admin'], ['client_portal', 'Client Portal'], ['backoffice', 'Backoffice']] as const).map(([platform, label]) => <label key={platform} className="flex items-center gap-2"><Checkbox checked={draft.platforms.includes(platform)} onCheckedChange={(checked) => togglePlatform(platform, checked === true)} /><span>{label}</span></label>)}</div></div>
