...
export default function SignupPage() {
  ...
  useEffect(() => {
    if (authLoading || isValidatingToken) return;
    if (!isAuthenticated || !profile || hasCompletedOnboarding || googleOAuthIntent?.source !== "signup") {
      return;
    }
    ...
    const continueGoogleSignup = async () => {
      ...
      if (waitlistLead && normalizeEmail(waitlistLead.email) !== normalizeEmail(user?.email)) {
        ...
        toast({
          ...
          description: "Use the invited email address to continue with this invitation.",
          variant: "destructive",
        });
        navigate(`/signup?invite=${invitationToken}`, { replace: true });
        return;
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: { redirectTo: `${window.location.origin}/signup${window.location.search}` },
      });
