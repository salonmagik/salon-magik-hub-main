...
type LastAuthMethod = "email" | "phone" | "google";
...
function persistRememberedEmail(email: string, enabled: boolean) {
  if (typeof window === "undefined") return;
  if (!enabled) {
...
function readLastAuthMethod(): LastAuthMethod | null {
  ...
  return value === "email" || value === "phone" || value === "google" ? value : null;
}

function persistLastAuthMethod(method: LastAuthMethod) {
  if (typeof window === "undefined") return;
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    setLastAuthMethod(readLastAuthMethod());
    if (remembered) {
		...
		>
			<AlertDialog open={showGoogleSignupPrompt}>
				...
				>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							It seems you don&apos;t have an account yet. Would you like to
							proceed to create one with your Google account?
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel onClick={handleCancelGoogleSignup}>
							Not now
						</AlertDialogCancel>
			...
			<div className="relative">
				...
				{lastAuthMethod === "google" && (
					<div className="z-50 absolute top-2 right-8">
						<span className="inline-flex items-center rounded-full border border-blue-500/40 bg-blue-50 px-2 py-1 text-xs font-normal text-blue-600">
							Last used
						</span>
					</div>
				)}
