...
export default function SettingsPage({ scope = "auto" }: SettingsPageProps) {
	...
	const activeBookingTheme: BookingThemeKey = entitlements?.has_ecommerce_theme
		? "ecommerce"
		: "default";
		...
		<Card>
			...
			<CardContent className="space-y-6">
				<Tabs
					value={bookingSubTab}
					onValueChange={(value) =>
						setBookingSubTab(value as BookingSettingsSubTab)
					}
				>
					<TabsList className="grid w-full max-w-md grid-cols-2">
						<TabsTrigger value="booking_config">Booking Config</TabsTrigger>
						<TabsTrigger value="themes_and_style">
							Themes &amp; Style
						</TabsTrigger>
					</TabsList>
				</Tabs>
					...
					<div className="space-y-6">
						<div className="grid gap-6 xl:grid-cols-[1.1fr_1.6fr]">
							<div className="space-y-4">
								...
								<div className="space-y-2">
									...
									/>
									<p className="text-xs text-muted-foreground">
										{bookingSettings.bookingPageBio.length}/280 characters.
										Appears below your salon name on the public storefront.
									</p>
								</div>
								...
								<div className="space-y-2">
									...
									<p className="text-xs text-muted-foreground">
										Both keeps your full catalog. Services or Products only
										gives your booking page a more focused layout for that one
										type — packages are always included either way.
									</p>
								</div>

								{/* ── E-commerce Theme Settings ──────────────────── */}
								<div className="space-y-3 rounded-xl border border-dashed p-4">
									<div>
										<p className="text-sm font-medium">E-commerce Theme</p>
										<p className="text-xs text-muted-foreground">Hero copy and about section — applies when the e-commerce theme is active.</p>
									</div>
									<div className="grid gap-3 sm:grid-cols-2">
										<div className="space-y-1.5">
											...
											<Input
												...
												value={bookingSettings.heroHeading}
												onChange={(e) => setBookingSettings((prev) => ({ ...prev, heroHeading: e.target.value }))}
												maxLength={80}
											/>
											<p className="text-xs text-muted-foreground">Large text. Defaults to your salon name.</p>
										</div>
										<div className="space-y-1.5">
											<Label className="text-xs">Hero Accent Line</Label>
											<Input
												placeholder="e.g. ... I come home"
												value={bookingSettings.heroTagline}
												onChange={(e) => setBookingSettings((prev) => ({ ...prev, heroTagline: e.target.value }))}
												maxLength={80}
											/>
											...
											<div className="grid grid-cols-2 gap-1.5">
												{(["Book Now", "Shop Now", "Get Started", "Explore"] as const).map((opt) => (
													<button
														key={opt}
														type="button"
														onClick={() => setBookingSettings((prev) => ({ ...prev, heroCTAPrimary: opt }))}
														className={cn(
															"rounded-md border px-2 py-1.5 text-xs font-medium transition-colors",
															bookingSettings.heroCTAPrimary === opt
																? "border-primary bg-primary text-primary-foreground"
																: "border-border bg-background hover:bg-muted/40",
														)}
													>
											...
											<div className="grid grid-cols-2 gap-1.5">
												{(["Our Services", "New In", "Browse Catalog", "View All"] as const).map((opt) => (
													<button
														key={opt}
														type="button"
														onClick={() => setBookingSettings((prev) => ({ ...prev, heroCTASecondary: opt }))}
														className={cn(
															"rounded-md border px-2 py-1.5 text-xs font-medium transition-colors",
															bookingSettings.heroCTASecondary === opt
																? "border-primary bg-primary text-primary-foreground"
																: "border-border bg-background hover:bg-muted/40",
														)}
													>
									...
									<div className="space-y-1.5">
										...
										<Textarea
											...
											value={bookingSettings.aboutText}
											onChange={(e) => setBookingSettings((prev) => ({ ...prev, aboutText: e.target.value }))}
											rows={3}
											maxLength={800}
										/>
										<p className="text-xs text-muted-foreground">{bookingSettings.aboutText.length}/800 characters.</p>
									</div>
								</div>
							</div>
							...
							<div className="space-y-4">
								<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
									<div>
										...
										<p className="mt-1 text-xs text-muted-foreground">
											Preview the real public booking page, then purchase and
											apply a paid theme when ready.
										</p>
									</div>
									<Badge variant="outline" className="w-fit">
										Current theme:{" "}
										{activeBookingTheme === "ecommerce"
											? "E-commerce"
											: "Default"}
									</Badge>
								</div>
									...
									<div className="rounded-xl border bg-card p-3 shadow-sm">
										<div className="mb-4 flex items-start justify-between gap-3">
											<div>
												...
												<p className="mt-1 text-sm text-muted-foreground">
													Clean appointment-first booking page with your salon
													branding.
												</p>
											</div>
											{activeBookingTheme === "default" && (
												<Badge>Applied</Badge>
											)}
										</div>
										...
										<div className="mt-4 flex flex-wrap gap-2">
											<Button
												type="button"
												variant="outline"
												onClick={() => openThemePreview("default")}
											>
											...
											<Button
												type="button"
												variant="ghost"
												onClick={() => openThemePreviewInNewTab("default")}
												disabled={!bookingUrl}
											>
									...
									<div className="rounded-xl border bg-card p-3 shadow-sm">
										<div className="mb-4 flex items-start justify-between gap-3">
											<div>
												...
												<p className="mt-1 text-sm text-muted-foreground">
													Shopify-inspired storefront styling adapted for
													bookable services, packages, vouchers, and products.
												</p>
											</div>
											<div className="flex flex-col items-end gap-2">
												{activeBookingTheme === "ecommerce" ? (
													...
													<Badge variant="secondary">Paid theme</Badge>
												)}
												<span className="text-xs text-muted-foreground">
													{ecommerceThemePricing
														? `${formatCurrency(ecommerceThemePricing, currentTenant?.currency || "USD")} / year`
														: "Annual billing"}
												</span>
											</div>
										</div>
										...
										<div className="mt-4 flex flex-wrap gap-2">
											<Button
												type="button"
												variant="outline"
												onClick={() => openThemePreview("ecommerce")}
											>
												<Eye className="mr-2 h-4 w-4" />
												Preview
											</Button>
											<Button
												type="button"
												variant="ghost"
												onClick={() => openThemePreviewInNewTab("ecommerce")}
												disabled={!bookingUrl}
											>
												<ExternalLink className="mr-2 h-4 w-4" />
												Open live preview
										...
										{!canPurchasePaidTheme &&
											activeBookingTheme !== "ecommerce" && (
												<p className="mt-2 text-xs text-muted-foreground">
													Upgrade from your trial first to purchase a paid
													theme.
												</p>
											)}
									</div>
						...
						<Dialog open={themePreviewOpen} onOpenChange={setThemePreviewOpen}>
							<DialogContent className="z-[200] sm:max-w-5xl">
								<DialogHeader>
									<DialogTitle>
										{themePreviewKey === "ecommerce"
											? "E-commerce theme"
											: "Default theme"}{" "}
										— how it looks to your customers
									</DialogTitle>
									<DialogDescription>
										This is a live simulation using your salon's name, brand
										color, banners, and content. Scroll down to see the full
										page including the service catalog.
									</DialogDescription>
								</DialogHeader>
								<div className="space-y-4">
										...
										activeBookingTheme !== "ecommerce" && (
											<div className="flex flex-col items-end gap-2">
												<Button
													type="button"
													onClick={purchaseThemeAddon}
													disabled={isPurchasingTheme || !canPurchasePaidTheme}
												>
													{isPurchasingTheme ? (
														<Loader2 className="mr-2 h-4 w-4 animate-spin" />
													) : (
