...
export function getCountryByCode(code: string): Country | undefined {
  return ALL_COUNTRIES.find((c) => c.code === code) || PRIORITY_COUNTRIES.find((c) => c.code === code);
}

// Get country by dial code
export function getCountryByDialCode(dialCode: string): Country | undefined {
  return ALL_COUNTRIES.find((c) => c.dialCode === dialCode) || PRIORITY_COUNTRIES.find((c) => c.dialCode === dialCode);
}
...
// Parse E.164 to get dial code and national number
export function parseE164(phone: string): { dialCode: string; nationalNumber: string } | null {
  ...
  for (const country of sortedCountries) {
    if (phone.startsWith(country.dialCode)) {
      return {
        ...
        nationalNumber: phone.slice(country.dialCode.length),
      };
    }
  }
