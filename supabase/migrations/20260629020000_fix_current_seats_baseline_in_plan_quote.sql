-- Found via live testing: compute_plan_configuration computed the chain
-- tenant's CURRENT base seat allotment as 12 * allowed_locations, but the
-- app's established source of truth (get_tenant_runtime_entitlements, used
-- by the Seats stat card and to seed the branches/seats inputs) computes it
-- as 12 * used_locations. Whenever allowed > used (i.e. a tenant has paid
-- for branch slots it hasn't opened yet), the quote's "current" baseline
-- disagreed with what was already on screen, so the page showed a phantom
-- price delta and an enabled "Update Billing" button even when the owner
-- hadn't changed anything. The REQUIRED (target) total still correctly uses
-- the requested branch count — only the CURRENT baseline needed to match.
...
begin
  ...
  if lower(v_current_plan_slug) = 'chain' then
    ...
    limit 1;
    -- Pricing is based on paid-for (allowed) locations, but the base seat
    -- allotment is based on used locations, matching
    -- get_tenant_runtime_entitlements exactly so this baseline agrees with
    -- what's already shown on the Seats stat card.
    v_current_base_staff := greatest(coalesce(v_chain_max_staff_per_location, 12), 1) * greatest(v_used_locations, 1);
