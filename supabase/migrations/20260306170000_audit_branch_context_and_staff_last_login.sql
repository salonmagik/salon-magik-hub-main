...
add column if not exists branch_location_id uuid references public.locations(id) on delete set null;
...
create function public.log_audit_event(
    ...
    _branch_location_id uuid DEFAULT NULL
)
returns uuid
language plpgsql
security definer
...
declare
    _audit_id uuid;
    _resolved_branch_location_id uuid := _branch_location_id;
begin
    if _resolved_branch_location_id is null then
      begin
        _resolved_branch_location_id := nullif(coalesce(_metadata->>'location_id', _metadata->>'branch_location_id'), '')::uuid;
      exception
        when invalid_text_representation then
          _resolved_branch_location_id := null;
      end;
    end if;
...
begin
  ...
  scoped_rows as (
    select
      ...
      au.last_sign_in_at as auth_last_login_at,
  ...
  select
    ...
    sr.auth_last_login_at as last_login_at,
    sr.profile_id,
    sr.full_name,
    sr.phone,
    sr.avatar_url,
