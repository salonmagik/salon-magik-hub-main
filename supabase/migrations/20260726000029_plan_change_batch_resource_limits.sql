-- backoffice's plan editor (create/edit plan) now lets super admins set
-- max_packages/max_vouchers alongside the existing max_services/max_products,
-- but the batch-apply function that actually writes plan_limits only knew
-- about the original four columns — any packages/vouchers cap set in the UI
-- would silently be dropped on rollout. Add them to the insert/upsert.
...
begin
  ...
  values (
    ...
    nullif(v_version.limits_json->>'max_packages', '')::integer,
    nullif(v_version.limits_json->>'max_vouchers', '')::integer,
