-- Lets a tenant decide whether their public storefront feels like a booking
-- app (services-focused), a shop (products-focused), or keeps today's
-- combined catalog ("both" — unchanged). This is independent of which paid
-- theme is active; packages are always shown regardless of mode.
alter table public.tenants
  add column if not exists storefront_mode text not null default 'both'
  check (storefront_mode in ('services', 'products', 'both'));

-- Recreate the view to expose the new column.
-- DROP ... CASCADE is required because the "Anon can read locations for booking"
-- policy on locations depends on this view. CASCADE drops that policy automatically; we
